...
import { ObjaxExecutionResult, ObjaxClassDefinition, ObjaxFieldDefinition } from './types'
          ...
          if (classDef.children.classBody) {
            for (const body of classDef.children.classBody) {
              if (body.children.fieldDeclaration) {
                ...
                const fieldName = fieldDecl.children.fieldName[0].image.slice(1, -1) // Remove quotes
