...
export class LinearObjaxParser {
  ...
  parse(code: string, existingInstances?: ObjaxInstanceDefinition[]): ObjaxExecutionResult {
    this.reset()
    ...
    for (const line of lines) {
      try {
        ...
        this.errors.push(`Parse error in "${line}": ${error instanceof Error ? error.message : String(error)}`)
      }
    }
    ...
    const newInstances = existingInstances 
      ? this.instances.filter(instance => !existingInstances.some(existing => existing.name === instance.name))
      : this.instances
  ...
  private parseLine(line: string) {
    ...
    // Non-existent instance configuration: try to catch and error
      ...
      this.errors.push(`Instance "${instanceName}" not found for configuration`)
    }
    ...
    // Event listener (new syntax): "instanceName addAction with event \"eventType\" and action blockName"
      ...
      console.log('New event listener detected:', tokens.map(t => t.value).join(' '))
  ...
  private tokenize(line: string): Token[] {
    ...
    // Second, extract square bracket arrays [...]
    ...
    while ((match = regex.exec(processedLine)) !== null) {
      ...
      if (value.startsWith('__ACTIONBLOCK_') && value.endsWith('__')) {
        const actionIndex = parseInt(value.replace('__ACTIONBLOCK_', '').replace('__', ''))
      ...
      if (value.startsWith('__SQUAREARRAY_') && value.endsWith('__')) {
        const arrayIndex = parseInt(value.replace('__SQUAREARRAY_', '').replace('__', ''))
      ...
      if (value.startsWith('__BLOCK_') && value.endsWith('__')) {
        const blockIndex = parseInt(value.replace('__BLOCK_', '').replace('__', ''))
  ...
  private isExistingClassExtension(tokens: Token[]): boolean {
    // "ClassName has field/method ..." when no current class context
    // Class names typically start with uppercase, instance names with lowercase
    const firstToken = tokens[0].value
    const startsWithUppercase = firstToken.charAt(0) === firstToken.charAt(0).toUpperCase()
  ...
  private handleClassDefinition(tokens: Token[]) {
    ...
    if (tokens.length < 4 || 
        ...
        tokens[2].value !== 'a' || 
        tokens[3].value !== 'Class') {
      throw new Error('Invalid class definition syntax. Use: ClassName is a Class')
    }
  ...
  private handleInstanceCreation(tokens: Token[]) {
    ...
    if (isIndex === -1 || aIndex === -1 || isIndex === 0 || aIndex >= tokens.length - 1) {
  ...
  private isAttemptedInstanceConfiguration(tokens: Token[]): boolean {
    ...
    const firstToken = tokens[0].value
    const startsWithLowercase = firstToken.charAt(0) === firstToken.charAt(0).toLowerCase()
  ...
  private handleMessagingCall(tokens: Token[]) {
    ...
    const methodCall: ObjaxMethodCall = {
      methodName,
      instanceName,
      keywordParameters: Object.keys(keywordParameters).length > 0 ? keywordParameters : undefined,
  ...
  private handleFieldAssignment(tokens: Token[]) {
    ...
    if (valueToken.type === 'ARRAY') {
      ...
      if (arrayContent.trim() === '') {
        ...
        const items = arrayContent.split(',').map(item => item.trim()).filter(Boolean)
  ...
  private handleNewEventListener(tokens: Token[]) {
    // "instanceName addAction with event \"eventType\" and action blockName"
    console.log('handleNewEventListener called with tokens:', tokens.map(t => t.value))
    ...
    console.log('Parsed event listener:', { instanceName, eventType, blockName })
  ...
  private isCreateMethod(tokens: Token[]): boolean {
    ...
    if (tokens.length >= 3 && tokens[0].value === 'create' && tokens[1].value === 'with') {
      return true
    }
    // Check for "ClassName create with ..." pattern
    if (tokens.length >= 4 && tokens[1].value === 'create' && tokens[2].value === 'with') {
      return true
    }
    return false
  }
  ...
  private handleCreateMethod(tokens: Token[]) {
    ...
    // Validate syntax
    if (tokens[startIndex].value !== 'create' || tokens[startIndex + 1].value !== 'with') {
    ...
    if (!className) {
      if (this.currentClass) {
        ...
        throw new Error('No class specified for create method and no class defined')
      }
    }
  ...
  private isBlockAssignment(tokens: Token[]): boolean {
    // "blockName is <block body>" or "blockName is <block body> with param1 and param2"
    return tokens.length >= 3 && 
           tokens[1].value === 'is' && 
  ...
  private handleBlockAssignment(tokens: Token[]) {
    ...
    // Extract block body (remove angle brackets)
  ...
  private parseCondition(conditionString: string): ObjaxCondition {
    ...
    if (tokens.length >= 3) {
      ...
      return {
        type: 'comparison',
        left,
        operator: operator as 'equal' | 'not_equal' | 'greater' | 'less' | 'greater_equal' | 'less_equal',
        right
      }
    }
